...
You can enable networking to download gridshift files on the fly, which are downloaded to a temporary directory by default (downstream packages can configure this behaviour).
...
You can examine the temporary directory with the (currently unexported) `libproj_tempdir`:
